...
// Limit targets per exam to reach 30%+ coverage (218+ subtopics total, currently 51 covered)
const SEED_TARGETS = {
  'UPSC': 75,
  'BPSC': 45,
  'State PCS': 25,
...
function getGeneratorForSubject(subject) {
  ...
  if (subjLower.includes('polity') || subjLower.includes('constitution') || subjLower.includes('governance')) {
    return {
      generate: (subtopic, exam) => {
        return {
          introduction: `This comprehensive module covers the essential aspects of "${subtopic}" which is a vital part of the ${subject} syllabus for the ${exam} exam. Understanding this topic is crucial for answering core governance and constitutional questions.`,
  ...
  return {
    generate: (subtopic, exam) => {
      const typeLabel = subjLower.includes('math') || subjLower.includes('quant') ? 'Mathematical Formulas & Steps' :
                        subjLower.includes('reasoning') ? 'Logical Reasoning Rules' : 'Scientific Principles';
      
      const detailsText = subjLower.includes('math') || subjLower.includes('quant')
        ...
        tables: [
          {
            ...
            rows: [
              ...
              ['Complex Case', 'Multi-step calculation', 'Break into smaller sub-problems', 'Eliminate extreme options first'],
...
async function seed() {
  ...
  const coveredSet = new Set(existingDocs.map(d => `${d.exam.toUpperCase()}:${d.subtopic.toUpperCase()}`));
  console.log(`📊 Currently covered subtopics in DB: ${coveredSet.size}`);
  ...
  for (const file of files) {
    ...
    const targetLimit = SEED_TARGETS[examName] || 20;

    // Get all subtopics from this syllabus file
    const subtopicsToProcess = [];
    if (data.subjects) {
    ...
    // Count how many we currently have covered for this exam
    const currentlyCoveredCount = existingDocs.filter(d => d.exam.toUpperCase() === examName.toUpperCase()).length;
    const neededCount = Math.max(0, targetLimit - currentlyCoveredCount);

    console.log(`\n📚 Exam: ${examName} | Target: ${targetLimit} | Covered: ${currentlyCoveredCount} | Needed: ${neededCount}`);

    if (neededCount === 0) {
      console.log(`  ✅ Target already met or exceeded.`);
      continue;
    }

    // Filter down to uncovered subtopics
    const uncoveredSubtopics = subtopicsToProcess.filter(item => {
      const key = `${examName.toUpperCase()}:${item.subtopic.toUpperCase()}`;
      return !coveredSet.has(key);
    });

    console.log(`  🔍 Found ${uncoveredSubtopics.length} uncovered subtopics. Seeding ${Math.min(neededCount, uncoveredSubtopics.length)}...`);

    let seededForThisExam = 0;
    for (const item of uncoveredSubtopics) {
      if (seededForThisExam >= neededCount) break;

      const generator = getGeneratorForSubject(item.subject);
      const generatedData = generator.generate(item.subtopic, examName);

      try {
        await LearningContent.findOneAndUpdate(
          { exam: examName, subtopic: item.subtopic },
          {
            $set: {
        ...
        console.error(`  ❌ Failed to seed "${item.subtopic}": ${err.message}`);
      }
    }

    console.log(`  ✅ Seeded ${seededForThisExam} subtopics for ${examName}.`);
  }

  console.log(`\n${'═'.repeat(60)}`);
  console.log(`📊 SEEDING COMPLETE | Total Seeded in Batch 3: ${totalSeeded}`);
