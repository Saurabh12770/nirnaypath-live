...
function ReportCard({ label, value, icon: Icon, color }) {
  return (
    <div className="glass-card" style={{ padding:24, textAlign:'center', border: '1px solid var(--color-border-base)' }}>
      ...
      <div style={{ fontSize:'1.8rem', fontWeight:800, color: 'var(--color-text-title-base)', fontFamily:'Outfit,sans-serif' }}>
        {typeof value === 'number' && String(value).includes('.') ? value.toFixed(1) : value}
      </div>
      <div style={{ color: 'var(--color-text-muted-base)', fontSize:13, marginTop:4, fontWeight: 500 }}>{label}</div>
    </div>
  );
}
...
export default function AdminPanel() {
  ...
  useEffect(() => {
    if (selectedSyllabusExam) {
      ...
      setSyllabusJson(prev => prev !== '' ? '' : prev);
      setSyllabusData(prev => prev !== null ? null : prev);
  ...
  const toggleRole = async (id) => {
    if (!confirm('Are you sure you want to change this user\'s privilege level? This will toggle their role between admin and student.')) return;
    try {
  ...
  const inputStyle = { background:'rgba(255,255,255,0.03)', border: '1px solid var(--color-border-base)', color: 'var(--color-text-title-base)', padding:'10px 14px', borderRadius:8, width:'100%', outline:'none', fontFamily:'Outfit,sans-serif', fontSize:14, marginBottom:12 };
    ...
    <AppLayout>
      <div style={{ padding:'28px 24px', maxWidth:1100, margin:'0 auto' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:28 }}>
          ...
          <div>
            <h1 style={{ fontSize:'1.6rem', fontWeight:800, color: 'var(--color-text-title-base)', fontFamily:'Outfit,sans-serif' }}>Admin Panel</h1>
            <p style={{ color: 'var(--color-text-muted-base)', fontSize:13 }}>Administrative override for system syllabus JSONs and study modules</p>
          </div>
        </div>

        {/* Tab bar */}
        <div style={{ display:'flex', gap:4, borderBottom: '1px solid var(--color-border-base)', marginBottom:28, overflowX:'auto' }}>
          {TABS.map(t => {
            const Icon = t.icon;
            return (
              <button key={t.id} id={`admin-tab-${t.id}`} onClick={() => setTab(t.id)}
                style={{ display:'flex', alignItems:'center', gap:6, padding:'12px 18px', background:'none', border:'none', cursor:'pointer', fontSize:13.5, fontWeight:tab===t.id?700:500,
                  color: tab===t.id?'var(--color-accent-primary)':'var(--color-text-muted-base)', borderBottom:tab===t.id?'2px solid var(--color-accent-primary)':'2px solid transparent', marginBottom:-1, transition:'all 0.2s', whiteSpace:'nowrap' }}>
        ...
        {tab === 'reports' && report && (
          <div>
            ...
            <div className="glass-card" style={{ padding:20, border: '1px solid var(--color-border-base)' }}>
              <p style={{ color: 'var(--color-text-muted-base)', fontSize:13.5, lineHeight: 1.6 }}>
                📊 <strong style={{ color:'#818cf8' }}>NirnayPath Engine v3.0</strong> is online. The relational indices for <code style={{ color: 'var(--color-text-title-base)' }}>questions</code> and <code style={{ color: 'var(--color-text-title-base)' }}>learningcontents</code> are active, resolving full mock tests and progress markers in sub-second timelines.
              </p>
            </div>
          </div>
        )}
        ...
        {tab === 'questions' && (
          <div>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
              <span style={{ color: 'var(--color-text-muted-base)', fontSize:13, fontWeight: 500 }}>Showing Page {qPage} of {qPages}</span>
            ...
            <div style={{ overflowX:'auto', border: '1px solid var(--color-border-base)', borderRadius: 12 }}>
                ...
                <tbody>
                  {questions.map(q => (
                    <tr key={q._id} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)' }}>
                      ...
                      <td style={{ padding:'14px 16px', color: 'var(--color-text-base)', fontSize:12.5 }}>{q.subject}</td>
                      <td style={{ padding:'14px 16px', color: 'var(--color-text-muted-base)', fontSize:12.5 }}>{q.topic}</td>
                      <td style={{ padding:'14px 16px', color: 'var(--color-text-title-base)', fontSize:12.5, maxWidth:260 }}>
        ...
        {tab === 'content' && (
          <div style={{ maxWidth:860 }}>
            <div className="glass-card" style={{ padding:28, border: '1px solid var(--color-border-base)', marginBottom: 20 }}>
              <h3 style={{ color: 'var(--color-text-title-base)', fontWeight:800, marginBottom:20, display:'flex', alignItems:'center', gap:8, fontSize: 16, fontFamily: 'Outfit, sans-serif' }}>
                <Plus size={18} color="#818cf8" /> Content Management System
              </h3>

              {/* Row 1: Exam Selector */}
              <div style={{ marginBottom: 16 }}>
                <label style={{ display:'block', color: 'var(--color-text-muted-base)', fontSize:11, fontWeight:600, marginBottom:6, letterSpacing:'0.05em', textTransform:'uppercase' }}>EXAM TRACK</label>
              ...
              {contentForm.exam && (
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:16 }}>
                  <div>
                    <label style={{ display:'block', color: 'var(--color-text-muted-base)', fontSize:11, fontWeight:600, marginBottom:6, letterSpacing:'0.05em', textTransform:'uppercase' }}>SUBJECT</label>
                  ...
                  <div>
                    <label style={{ display:'block', color: 'var(--color-text-muted-base)', fontSize:11, fontWeight:600, marginBottom:6, letterSpacing:'0.05em', textTransform:'uppercase' }}>TOPIC</label>
              ...
              {contentForm.topic && (
                <div style={{ display:'flex', gap:14, marginBottom:20, alignItems:'flex-end' }}>
                  <div style={{ flex:1 }}>
                    <label style={{ display:'block', color: 'var(--color-text-muted-base)', fontSize:11, fontWeight:600, marginBottom:6, letterSpacing:'0.05em', textTransform:'uppercase' }}>SUBTOPIC</label>
              ...
              {[['introduction','Introduction Summary (1-2 paragraphs)'],['detailedExplanation','Detailed Explanation (Markdown supported: ## ## ### **bold**)'],['revisionNotes','Quick Revision Notes (bullet points)']].map(([field, label]) => (
                <div key={field}>
                  <label style={{ display:'block', color: 'var(--color-text-muted-base)', fontSize:11, fontWeight:600, marginBottom:4, letterSpacing:'0.05em', textTransform:'uppercase' }}>{label}</label>
              ...
              <div style={{ marginBottom:20, paddingTop:16, borderTop:'1px solid rgba(255,255,255,0.05)' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
                  <label style={{ color: 'var(--color-text-muted-base)', fontSize:11, fontWeight:600, letterSpacing:'0.05em', textTransform:'uppercase' }}>IMPORTANT FACTS ({contentForm.importantFacts.length})</label>
              ...
              <div style={{ marginBottom:20, paddingTop:16, borderTop:'1px solid rgba(255,255,255,0.05)' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
                  <label style={{ color: 'var(--color-text-muted-base)', fontSize:11, fontWeight:600, letterSpacing:'0.05em', textTransform:'uppercase' }}>SUMMARY TABLES ({contentForm.tables.length})</label>
                ...
                {contentForm.tables.map((table, ti) => (
                  <div key={ti} style={{ background:'rgba(255,255,255,0.01)', border: '1px solid var(--color-border-base)', borderRadius:10, padding:16, marginBottom:14 }}>
                    ...
                    <div style={{ overflowX:'auto' }}>
                      <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12.5 }}>
                        <thead><tr>{table.headers.map((h, hi) => (
                          <th key={hi} style={{ border: '1px solid var(--color-border-base)', padding:'8px 10px', background:'rgba(99,102,241,0.08)', color:'#818cf8', fontWeight:700, minWidth:120 }}>{h}</th>
                        ))}</tr></thead>
                        <tbody>{table.rows.map((row, ri) => (
                          <tr key={ri}>{row.map((cell, ci) => (
                            <td key={ci} style={{ border: '1px solid var(--color-border-base)', padding:4 }}>
              ...
              <div style={{ marginBottom:20, paddingTop:16, borderTop:'1px solid rgba(255,255,255,0.05)' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
                  <label style={{ color: 'var(--color-text-muted-base)', fontSize:11, fontWeight:600, letterSpacing:'0.05em', textTransform:'uppercase' }}>PREVIOUS YEAR QUESTIONS ({contentForm.pyqs.length})</label>
                ...
                {contentForm.pyqs.map((q, qi) => (
                  <div key={qi} style={{ background:'rgba(255,255,255,0.01)', border: '1px solid var(--color-border-base)', borderRadius:10, padding:16, marginBottom:14 }}>
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                      <div>
                        <label style={{ color: 'var(--color-text-muted-base)', fontSize:10, fontWeight:600 }}>Year</label>
                        <input value={q.year} onChange={e => updatePYQ(qi,['year'],e.target.value)} style={{ ...inputStyle, marginBottom:0 }} placeholder="2022" />
                      </div>
                      <div>
                        <label style={{ color: 'var(--color-text-muted-base)', fontSize:10, fontWeight:600 }}>Correct Option (0–3)</label>
                    ...
                    <label style={{ color: 'var(--color-text-muted-base)', fontSize:10, fontWeight:600, marginTop:8, display:'block' }}>Question (English)</label>
                    ...
                    <label style={{ color: 'var(--color-text-muted-base)', fontSize:10, fontWeight:600 }}>Question (Hindi)</label>
                    ...
                    <label style={{ color: 'var(--color-text-muted-base)', fontSize:10, fontWeight:600, marginTop:6, display:'block' }}>Explanation (English)</label>
        ...
        {tab === 'syllabus' && (
          <div style={{ maxWidth: 850 }}>
            <div className="glass-card" style={{ padding:28, border: '1px solid var(--color-border-base)' }}>
              <h3 style={{ color: 'var(--color-text-title-base)', fontWeight:800, marginBottom:20, display:'flex', alignItems:'center', gap:8, fontSize: 16, fontFamily: 'Outfit, sans-serif' }}>
                <Code size={18} color="#818cf8" /> Syllabus Structure Editor
              </h3>
              
              <div style={{ marginBottom: 20 }}>
                <label style={{ display:'block', color: 'var(--color-text-muted-base)', fontSize:11, fontWeight:600, marginBottom:6, letterSpacing:'0.05em' }}>SELECT EXAM TRACK</label>
              ...
              {selectedSyllabusExam ? (
                <div>
                  {/* Mode Toggle Bar */}
                  <div style={{ display: 'flex', gap: 12, marginBottom: 20, borderBottom: '1px solid var(--color-border-base)', paddingBottom: 16 }}>
                  ...
                  {editorMode === 'raw' ? (
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                        <label style={{ color: 'var(--color-text-muted-base)', fontSize:11, fontWeight:600, letterSpacing:'0.05em' }}>RAW SYLLABUS JSON</label>
                      ...
                      <textarea
                        ...
                        style={{ ...taStyle, minHeight: 320, fontFamily: 'monospace', fontSize: 13, background: '#070a13', color: '#818cf8', border: '1px solid var(--color-border-base)' }}
                      />
                    </div>
                  ) : (
                    <div>
                      {/* Visual Tree Builder */}
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                        <label style={{ color: 'var(--color-text-muted-base)', fontSize:11, fontWeight:600, letterSpacing:'0.05em' }}>VISUAL SYLLABUS HIERARCHY</label>
                      ...
                      {syllabusData && syllabusData.subjects && syllabusData.subjects.length > 0 ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginBottom: 20 }}>
                          {syllabusData.subjects.map((subj, sIdx) => {
                            ...
                            return (
                              <div key={sIdx} className="glass-card" style={{ padding: '16px', border: '1px solid var(--color-border-base)', background: 'rgba(255,255,255,0.01)', borderRadius: '10px' }}>
                                {/* Subject Header */}
                                <div style={{ display: 'flex', alignItems: 'center', gap: 10, justifyContent: 'space-between', flexWrap: 'wrap' }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: '250px' }}>
                                    ...
                                    <input
                                      ...
                                      style={{ ...inputStyle, marginBottom: 0, fontWeight: 700, fontSize: 15, background: 'transparent', border: 'none', borderBottom: '1px solid var(--color-border-base)', padding: '2px 4px', width: '90%' }}
                                    />
                                  </div>
                                  <div style={{ display: 'flex', gap: 8 }}>
                                ...
                                {isSubjExpanded && (
                                  <div style={{ paddingLeft: '18px', borderLeft: '1px dashed rgba(255,255,255,0.08)', marginTop: '14px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                                    {subj.topics && subj.topics.length > 0 ? (
                                            ...
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 10, justifyContent: 'space-between', flexWrap: 'wrap' }}>
                                              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: '200px' }}>
                                                ...
                                                <input
                                                  ...
                                                  style={{ ...inputStyle, marginBottom: 0, fontWeight: 600, fontSize: 13.5, background: 'transparent', border: 'none', borderBottom: '1px solid var(--color-border-base)', padding: '2px 4px', width: '90%' }}
                                                />
                                              </div>
                                            ...
                                            {isTopicExpanded && (
                                              <div style={{ paddingLeft: '18px', borderLeft: '1px dashed rgba(255,255,255,0.08)', marginTop: '10px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                                                {topic.subtopics && topic.subtopics.length > 0 ? (
                                                    ...
                                                    {topic.subtopics.map((sub, subIdx) => (
                                                      <div key={subIdx} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                        <input
                                                          ...
                                                          style={{ ...inputStyle, marginBottom: 0, fontSize: 13, background: 'rgba(255,255,255,0.02)', border: '1px solid var(--color-border-base)', borderRadius: '6px', padding: '6px 10px', flex: 1 }}
                                                        />
                                                  ...
                                                  <span style={{ color: 'var(--color-text-muted-base)', fontSize: 12, fontStyle: 'italic' }}>No subtopics added yet. Click "Add Subtopic".</span>
                                                )}
                                              </div>
                                            )}
                                          </div>
                                      ...
                                      <span style={{ color: 'var(--color-text-muted-base)', fontSize: 12, fontStyle: 'italic' }}>No topics added yet. Click "Add Topic".</span>
                                    )}
                                  </div>
                                )}
                              </div>
                        ...
                        <div style={{ padding: '30px', textAlign: 'center', color: 'var(--color-text-muted-base)', border: '1px dashed rgba(255,255,255,0.1)', borderRadius: '10px' }}>
                ...
                <div style={{ padding: '40px 0', textAlign: 'center', color: 'var(--color-text-muted-base)' }}>
        ...
        {/* Users Tab */}
        {tab === 'users' && (
          <div style={{ border: '1px solid var(--color-border-base)', borderRadius: 12, overflow: 'hidden' }}>
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--color-border-base)' }}>
              ...
              <tbody>
                {users.map(u => (
                  <tr key={u._id} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)' }}>
                    <td style={{ padding:'14px 16px', color: 'var(--color-text-title-base)', fontSize:13, fontWeight: 600 }}>{u.name}</td>
                    <td style={{ padding:'14px 16px', color: 'var(--color-text-muted-base)', fontSize:13 }}>{u.email}</td>
                    <td style={{ padding:'14px 16px' }}>
                    ...
                    <td style={{ padding:'14px 16px', color: 'var(--color-text-muted-base)', fontSize:12.5 }}>{new Date(u.createdAt).toLocaleDateString(undefined, {month:'short', day:'numeric', year:'numeric'})}</td>
                    <td style={{ padding:'14px 16px' }}>
                      <button id={`toggle-role-${u._id.slice(-6)}`} onClick={() => toggleRole(u._id)}
                        style={{ background:'rgba(255,255,255,0.03)', border: '1px solid var(--color-border-base)', borderRadius:8, padding:'6px 12px', color:'#a5b4fc', cursor:'pointer', fontSize:12, display:'flex', alignItems:'center', gap:6, fontWeight: 600 }}
